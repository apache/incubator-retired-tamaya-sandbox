...
public class ConfigSourceOrderingReader implements MetaConfigurationReader{
    ...
    public void read(Document document, ConfigBuilder configBuilder) {
        NodeList nodeList = document.getDocumentElement().getElementsByTagName("source-order");
        if(nodeList.getLength()==0){
            LOG.finer("No config source ordering defined.");
            return;
        }
        if(nodeList.getLength()>1){
            throw new IllegalArgumentException("Only one source order can be applied.");
        }
        ...
        LOG.finer("Sorting config sources using comparator: " + comparator.getClass().getName());
        TamayaConfigBuilder.from(configBuilder).sortSources(comparator);
    }
